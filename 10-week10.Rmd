...
gmp_month <- read_csv(url(data_url))
...
Cities release their own data. Here, we will be looking at crimes from Dallas, Texas. [You can see more about these data, and find the download link to the data dictionary here](https://www.dallasopendata.com/Public-Safety/Police-Bulk-Data/ftja-9jxd).  Download the file and open bring it into R. 

```{r, eval=FALSE}
library(readxl)
data_url <-"https://github.com/maczokni/crimemapping_textbook_bookdown/raw/master/data/dallas_burg_correct.xlsx"
download.file(data_url, destfile = "D://dallas.xlsx", method= "wininet", mode = "wb")
dallas <- read_xlsx("D://dallas.xlsx")

```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
library(readxl)
dallas <- read_excel("data/dallas_burg_correct.xlsx")
```
...
library(lubridate)
names(dallas)
dallas$year <- year(dallas$Date.of.Occurrence)
dallas$month <- month(dallas$Date.of.Occurrence, label = TRUE, abbr=FALSE, locale = "UK")
dallas$day <- day(dallas$Date.of.Occurrence)
dallas$wday <- wday(dallas$Date.of.Occurrence, label = TRUE, abbr=FALSE, locale = "UK")
...
Decomposing a time series means separating it into its constituent components, which are usually a trend component and an irregular component, and if it is a seasonal time series, a seasonal component. Let's have a look at the burglary data from Dallas.

First we will need to get the data into shape.
